...
<!-- 
 ...
 -->
  ...
  <xsl:template match="explanation/account">
    <td>
      <a name="{../symbol/@link}"><xsl:apply-templates/></a>
    </td>
  </xsl:template>
  ...
  <xsl:template match="aliasmnem">
    <h3 class="alias-section">
      <a name="{@id}"><xsl:value-of select="@heading"/> alias</a>
    </h3>
    <xsl:apply-templates/>
  </xsl:template>
  ...
  <xsl:template match="xref">
    <a class="armarm-xref" title="Reference to ARM ARM section"><xsl:value-of select="."/></a>
  </xsl:template>
