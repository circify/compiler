...
<!-- for encoding index with group table: this version of the script
     produces a browser view with a high-level group table and
     sub-tables in each group section.  Use encodingindex.xsl to see a
     single big main table. -->
            ...
            <xsl:if test="@iclass">
              ...
              <xsl:if test="$HASVERSION != ''">                
                ...
                <xsl:choose>
                  ...
                  was introduced. -->
                  <xsl:when test="$ROW/@arch_version">
                    <xsl:text>ARM</xsl:text>
                    <xsl:value-of select="$ROW/@arch_version"/>
                  </xsl:when>
                  <xsl:otherwise>
                    <xsl:text>-</xsl:text>
                  </xsl:otherwise>
  ...
  <xsl:template name="decode_unpredictables">
    ...
    <p>
      ...
      <xsl:element name="a">
        ...
        <xsl:attribute name="title"><xsl:text>Reference to ARM ARM section</xsl:text></xsl:attribute>
  ...
  <!-- The "groups" element is used to wrap the new high-level groups
       table, to make it easy to ignore.  We're going to render it. -->
  <xsl:template match="groups">
    <h1 class="topleveltable">
      <xsl:text>Top-level Encoding Groups for </xsl:text>
      <xsl:value-of select="/encodingindex/@instructionset"/>
    </h1>
    <xsl:apply-templates/>
  </xsl:template>

  <!-- the "maintable" can be rendered where it is found, but when
       [class="allclasses"] it is ignored when seen, and subsequently
       mined for data for tables under each main-group heading.
  -->
  <xsl:template match="maintable">
    <xsl:choose>
      <xsl:when test="@class='allclasses'">
        <!-- omit big main encoding table in this view, we get an
             encoding table per top-level group -->
      </xsl:when>
      <xsl:otherwise>
        <!-- the top-level group itself is a maintable -->
  ...
  <!-- "funcgroupheader" provides a heading for a group of instruction
        tables in the same functional group, but it also provides a
        trigger for the renderer to write out this group's subset of
        the maintable.
  -->
  <xsl:template match="encodingindex/funcgroupheader">
    <xsl:variable name="id" select="@id"/>
    <a name="{@id}"></a>
    <xsl:for-each select="//encodingindex/mappings/mapping[@from = $id]">
      <!-- where multiple encodings are described, need to include 
           anchors for them all. -->
      <a name="{@to}"></a>
    </xsl:for-each>
    <h2 class="encfuncgroup"><xsl:value-of select="."/> Group</h2>
    <!-- each encoding group gets a decode table which is a slice of
         the 'maintable' element in the XML -->
    <xsl:for-each select="//maintable[@class='allclasses']">
      <!-- where more than one table, need to output the instruction
           set to differentiate them. -->
      <xsl:if test="@isa">
        <h4><xsl:value-of select="@isa"/></h4>
      </xsl:if>
      <xsl:variable name="idforisa">
        <xsl:choose>
          <xsl:when test="@isa and not(//encodingindex/mappings/mapping[@from = $id]/@from_isa = @isa)">
            <xsl:value-of select="//encodingindex/mappings/mapping[@from = $id]/@to"/>
          </xsl:when>
          <xsl:otherwise>
      ...
      <table class="maintable">
        <thead>
          <!-- just replicate the headings -->
          <xsl:for-each select="./tableheader">
            <xsl:apply-templates/>
          </xsl:for-each>
        </thead>
        <tbody>
          <!-- select only the rows which match the desired ID -->
          <xsl:for-each select="./tablebody/tr[@class='maintable'][@groupid=$idforisa]">
            <tr>
              <xsl:apply-templates/>
            </tr>
          </xsl:for-each>
