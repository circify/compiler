...
data PfUnOp = PfNeg | PfRecip | PfTwoPow deriving (Show, Read, Ord, Eq, Typeable, Generic, Hashable)
data PfBinOp = PfLt deriving (Show, Read, Ord, Eq, Typeable, Generic, Hashable)
...
data Term s where
    ...
    -- Prime field terms
    PfBinPred  ::KnownNat n => !PfBinOp -> !(TermPf n) -> !(TermPf n) -> TermBool
