...
mkLoop :: (ControlTerm t, Monad m) => t -> t -> t -> Control t -> Control t -> Control t -> Control t -> StateT (Map String Natural) m (Control t)
            ...
            case (pullbackLoop body) of
                ...
                (body1, For { body = body2, end = m, iter = i, .. }, body3) ->  -- Inner loop (For)
                    do
                        ...
                        flatloop <- mkLoop
                                      (lit . toInteger $ maxIteration)              -- Maximum iteration used for while loops as upper-bound
